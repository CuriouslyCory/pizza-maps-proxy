...
function getHandler(options, proxy) {
  var corsAnywhere = {
    ...
    reqWhitelist: [],               // If non-empty, requests to a domain must match one in the list.
  ...
  var isValidRequestLocation = function(location) {
    console.log(location);
    var pass = false;
    // none set, pass check
    if(corsAnywhere.reqWhitelist.length < 1)
      return true;
    
    // check whitelist against location
    corsAnywhere.reqWhitelist.forEach(whiteListLoc => {
      console.log(whiteListLoc)
      if(location.includes(whiteListLoc))
        pass = true;
    });
  ...
  return function(req, res) {
    ...
    if(!isValidRequestLocation(location.hostname)){
      res.writeHead(404, 'Invalid request location', cors_headers);
      res.end('Invalid request location: ' + location.hostname);
      return;
    }
